...
| **`h`, `help`** | Command | `[command]` | **Reserved for Help:** These opcodes, along with `?`, are reserved for invoking a help system. The `debug_cli` example provides a reference implementation. See the [Debug CLI Syntax](./DEBUG_CLI_SYNTAX.en.md) for details. |
